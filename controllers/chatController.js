...
const sendMessage = async (req, res) => {
  ...
  try {
    ...
    console.log("NEW MESSAGE:", newMessage.toJSON());

    // Delete sender conversation cache
    await redisClient.del(`chat:messages:${senderId}:${receiver}`);

    // Delete receiver conversation cache
    await redisClient.del(`chat:messages:${receiver}:${senderId}`);
...
const getMessages = async (req, res) => {
  try {
    ...
    const cacheKey = `chat:messages:${currentUserId}:${receiverId}`;
...
const markMessagesAsSeen = async (req, res) => {
  try {
    ...
    await redisClient.del(`chat:messages:${receiverId}:${senderId}`);

    await redisClient.del(`chat:messages:${senderId}:${receiverId}`);
...
const getUnreadCounts = async (req, res) => {
  try {
    ...
    unreadMessages.forEach((chat) => {
      const senderId = String(chat.userId);
    ...
    console.log("Get Unread Counts Error:", err);
